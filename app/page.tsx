...
export default function MainPage() {
  ...
  useEffect(() => {
    fetch('/trade-barriers/api/commitments')
      ...
      .then(data => {
        setData(data);
        setFilteredData(data); // *** maybe call the filter function here?
  ...
  const handleFiltersChange = useCallback((filteredCommitments: Commitment[]) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6 mb-8">
          <Card className="bg-white border-[#d3c7b9]">
            <CardContent className="p-6">
              <div className="text-2xl font-bold text-[#8b2332]">{stats.total}</div>
              <div className="text-sm text-gray-600">Total Commitments</div>
            </CardContent>
          </Card>
          <Card className="bg-white border-[#d3c7b9]">
            <CardContent className="p-6">
              <div className="text-2xl font-bold text-[#8b2332]">{stats.awaitingSponsorship}</div>
              <div className="text-sm text-gray-600">Awaiting Sponsorship</div>
            </CardContent>
          </Card>
          <Card className="bg-white border-[#d3c7b9]">
            <CardContent className="p-6">
              <div className="text-2xl font-bold text-[#8b2332]">{stats.underNegotiation}</div>
              <div className="text-sm text-gray-600">Under Negotiation</div>
            </CardContent>
          </Card>
          <Card className="bg-white border-[#d3c7b9]">
            <CardContent className="p-6">
              <div className="text-2xl font-bold text-[#8b2332]">{stats.partiallyImplemented}</div>
              <div className="text-sm text-gray-600">Partially Implemented</div>
            </CardContent>
          </Card>
          <Card className="bg-white border-[#d3c7b9]">
            <CardContent className="p-6">
              <div className="text-2xl font-bold text-[#8b2332]">{stats.implemented}</div>
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {filteredData.map(item => {
                ...
                const isDueSoon = daysUntilDeadline !== null && daysUntilDeadline <= 30 && daysUntilDeadline >= 0;
                  ...
                  >
                    <CardHeader className="pb-4">
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                        <div className="flex-1">
                          <CardTitle className="text-xl text-[#272727] mb-2">{item.title}</CardTitle>
                        ...
                        <div className="flex flex-col items-end gap-2">
                          ...
                          {daysUntilDeadline !== null && (
                            <div className={`text-sm px-3 py-1 rounded-full border ${isOverdue
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {item.jurisdictions.slice(0, 3).map(jurisdiction => (
                            <span key={jurisdiction} className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded">
                              {jurisdiction}
                            </span>
                          ))}
            ...
            {filteredData.length === 0 && (
              <Card className="bg-white border-[#d3c7b9] text-center py-12">
                <CardContent>
                  <div className="text-gray-500 text-lg">No commitments match your filters</div>
